...
class ResponsivenessActionTests: XCTestCase {
  ...
  func testUndriven() {
    ...
    expect(result.indiv.responsiveness) == 0.5
  ...
  func testPositiveHigh() {
    ...
    expect(result.indiv.responsiveness) == 1
  ...
  func testPositiveLow() {
    ...
    expect(result.indiv.responsiveness) ≈ 0.8808
  ...
  func testNegativeHigh() {
    var result = ActionResult(indiv: individual, killed: [], responseCurve: { $0 * 2 })
    sut.apply(to: &result, level: -50, on: grid, with: parameters)
    expect(result.indiv.responsiveness) == 0
  ...
  func testNegativeLow() {
    ...
    expect(result.indiv.responsiveness) ≈ 0.0024
