...
struct BoundaryDistanceSensor: Sensor {
  ...
  func get(for individual: Individual, on world: World) -> Double {
    ...
    case .x:
      ...
      let minDistanceX = min(individual.loc.x, (grid.size.width - individual.loc.x) - 1)
      return Double(minDistanceX) / (Double(grid.size.width) / 2.0)
    case .y:
      ...
      let minDistanceY = min(individual.loc.y, (grid.size.height - individual.loc.y) - 1)
      return Double(minDistanceY) / (Double(grid.size.height) / 2.0)
    case .both:
      ...
      let distanceX = min(individual.loc.x, (grid.size.width - individual.loc.x) - 1)
      let distanceY = min(individual.loc.y, (grid.size.height - individual.loc.y) - 1)
      let closest = min(distanceX, distanceY)
      let maxPossible = max(grid.size.width / 2 - 1, grid.size.height / 2 - 1)
