...
extension Individual {
  mutating func feedForward(on world: World) -> [(action: Action, level: Double)] {
    ...
    for conn in nnet.connections {
      ...
      if case .sensor = conn.sourceType {
        if world.parameters.sensors.count > conn.sourceNum {
          let sensor = world.parameters.sensors[conn.sourceNum]
          inputVal = sensor.get(for: self, on: world)
        }
      } else {
    ...
    return world.parameters.actions.lazy.enumerated().map { (action: $1, level: levels[$0, default: 0.0]) }
  }
}
